...
//                    "/Users/jamie/IdeaProjects/AdventCalendarPuzzles/out/production/AdventCalendarPuzzles/com/daySixteen/exampleData.txt"
        ...
        for (int k = 0; k < 20; k++) {
            switch(k) {
                ...
                case 4:
                    ...
                    break;
                case 5:
                    indexOfFieldAndValues.put(k, five_index);
                    break;
                case 6:
    ...
    private Map<Integer, Integer> setRulesToIndex(List<List<Integer>> listOfPossibilities) {
        Map<Integer, Integer> ruleVsIndex = new HashMap<>();
        for (int j = 0; j < listOfPossibilities.size(); j++) {
            if (listOfPossibilities.size() == 1) {
                ruleVsIndex.put(listOfPossibilities.get(j).get(0), j);
            }
        }
        for (int i = 2; i < listOfPossibilities.size() + 1; i++) {
            for (int j = 0; j < listOfPossibilities.size(); j++) {
                if (listOfPossibilities.get(j).size() == i) {
                    List<Integer> valuesOfPossibilities = listOfPossibilities.get(j);
                    Set<Integer> keys = ruleVsIndex.keySet();
                    for (Integer key : keys) {
                        valuesOfPossibilities.remove(Integer.valueOf(key));
                    }
                    ruleVsIndex.put(valuesOfPossibilities.get(0), j);
                }
            }
        }
        return ruleVsIndex;
    ...
    public void test() {
        ...
        List<List<Integer>> listOfPossibilities = new ArrayList<>();
        for(int i = 0; i < indexOfFieldAndValues.size(); i++) {
            List<Integer> listOfIndexVsRules = findField(i);
            listOfPossibilities.add(listOfIndexVsRules);
//            System.out.println("COLUMN : " + i + " - " + indexOfFieldAndValues.get(i));
//            System.out.println("POSSIBLE RULES : " + listOfIndexVsRules);
//            System.out.println("  ");
        }
        System.out.println("LIST OF POSS SIZE " + listOfPossibilities.size());
        Map<Integer, Integer> mapOfRules = setRulesToIndex(listOfPossibilities);
